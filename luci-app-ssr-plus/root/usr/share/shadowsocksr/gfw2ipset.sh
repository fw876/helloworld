...
if [ "$ABBLOCK" == "1" ] && [ -f "/etc/ssr/ad.conf" ]; then
	cp -f /etc/ssr/ad.conf /tmp/dnsmasq.ssr/
	awk '!/^$/&&!/^#/{printf("address=/%s/''\n",$0)}' /etc/ssr/deny.list >>/tmp/dnsmasq.ssr/ad.conf
	for line in $(cat /etc/ssr/allow.list); do sed -i "/$line/d" /tmp/dnsmasq.ssr/ad.conf; done
else
	awk '!/^$/&&!/^#/{printf("address=/%s/''\n",$0)}' /etc/ssr/deny.list >/tmp/dnsmasq.ssr/denylist.conf
	for line in $(cat /etc/ssr/allow.list); do sed -i "/$line/d" /tmp/dnsmasq.ssr/denylist.conf; done
fi
